...
## ----beta4, echo = FALSE, fig.width = 12, fig.height = 5, out.width = "80%"---------------------------------------------
p <- seq(0, 1, length = 100)
...
## ----work-gif, echo = FALSE, out.width = "400px"------------------------------------------------------------------------
knitr::include_graphics("figures/not_gonna_work.gif")


## ----sampling1, eval = TRUE, echo = TRUE, fig.width = 6, fig.height = 3, out.width = "50%", `code-line-numbers` = "|5"----
#| output-location: fragment
...
data.frame(samples = samples) %>%
  ...
  geom_histogram(aes(y = after_stat(density) ), fill = "purple", alpha = 0.25, bins = 50) +
...
ggplot(df, aes(x = theta, y = posterior, colour = NULL) ) +
  ...
  geom_vline(xintercept = 0.2, color = "white") +
    geom_vline(xintercept = 0.4, color = "white") +
  geom_area(
...
p_theta <- dbeta(x = p_grid, shape1 = 3, shape2 = 10)
samples <- sample(x = p_grid, size = 1e4, replace = TRUE, prob = p_theta)
...
posterior_plot(samples = samples, rope = c(0.49, 0.51) ) + labs(x = expression(theta) )
...
# prior uniforme
prior <- rep(x = 1, times = grid_size)

# calcul de la vraisemblance
likelihood <- dbinom(x = nb_success, size = nb_trial, prob = p_grid)
...
str(data2)
nb_trial2 <- length(data2$trial) # nombre d'essais
nb_success2 <- sum(data2$side) # nombre de succès


## ----eval = TRUE, echo = TRUE, fig.width = 10, fig.height = 5-----------------------------------------------------------
mode1 <- find_mode(samples)
prior2 <- dbeta(x = p_grid, shape1 = mode1 * (nb_trial - 2) + 1, shape2 = (1 - mode1) * (nb_trial - 2) + 1)
...
## ----eval = TRUE, echo = TRUE, fig.width = 10, fig.height = 6, dev = "png", dpi = 200-----------------------------------
likelihood2 <- dbinom(x = nb_success2, size = nb_trial2, prob = p_grid)
