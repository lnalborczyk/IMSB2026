...
for (i in 1:nsims) {
    ...
    t[i] <- t.test(men2, women2)$statistic # on stocke la t-valeur dans t[i]
...
data.frame(x = c(-20, 20) ) %>%
    ...
    stat_function(fun = dnorm, args = list(mean = 0, sd = 2), size = 1.25) +
    stat_function(fun = dnorm, args = list(mean = 0, sd = 5), size = 1.25) +
    stat_function(fun = dnorm, args = list(mean = 0, sd = 10), size = 1.25) +
...
# "vraie distribution" p : Normal(0, 1)
true_mu <- 0
true_sd <- 1
x <- rnorm(n = n, mean = true_mu, sd = true_sd)

# modèles candidats q_k = Normal(mu_k, sd_k)
models <- data.frame(
    model = c("q1", "q2", "q3", "q4"),
    mu = c(0, 0, 2, 0),
    sd = c(1, 1.2, 1, 0.7)
...
all_models <- bind_rows(
    data.frame(model = "p", mu = 0, sd = 1),
    models
    )

x_min <- min(all_models$mu - 4 * all_models$sd)
x_max <- max(all_models$mu + 4 * all_models$sd)

grid <- tidyr::expand_grid(
    model = all_models$model,
    x = seq(x_min, x_max, length.out = 500)
    ) %>%
    left_join(all_models, by = "model") %>%
    mutate(y = dnorm(x, mean = mu, sd = sd) )

# niveaux dans l'ordre souhaité
levels_all <- c("p", "q1", "q2", "q3", "q4")

# palette ggplot par défaut pour 5 catégories (hue)
pal_all <- setNames(scales::hue_pal()(length(levels_all)), levels_all)

grid %>%
    ggplot(aes(x = x, y = y, color = model, fill = model) ) +
    geom_area(alpha = 0.5, linewidth = 1, show.legend = FALSE) +
    labs(x = "x", y = "Density", color = "Model") +
...
## ----eval = TRUE, echo = FALSE---------------------------------------------------------------------------------
# KL exacte entre deux normales: N(m0,s0) et N(m1,s1)
...
## ----eval = TRUE, echo = TRUE----------------------------------------------------------------------------------
# On sait calculer l'entropie exacte d'une distribution Gaussienne
# (e.g., https://gregorygundersen.com/blog/2020/09/01/gaussian-entropy/),
# et l'entropie croisée de deux distributions Gaussiennes.
res <- models %>%
    mutate(
        # KL exacte
        KL_exact = mapply(KL_normal, true_mu, true_sd, mu, sd),
        # H(p,q) exacte
        H_exact = mapply(H_cross_normal, true_mu, true_sd, mu, sd),
        # Estimation empirique de H(p,q)
        H_est = sapply(seq_along(mu), function (i) {
...
## ----eval = TRUE, echo = FALSE---------------------------------------------------------------------------------
res %>%
    ggplot(aes(x = KL_exact, y = H_est, label = model) ) +
    geom_point(size = 3) +
    geom_line() +
    geom_label(
        aes(fill = model),
        colour = "white",
        nudge_y = 0.05 * diff(range(res$H_est) ),
        show.legend = FALSE
        ) +
    labs(
        x = "KL Divergence (exacte)",
        y = "Negative log-score (entropie croisée estimée)"
        ) +
    scale_color_manual(values = pal_all) +
    scale_fill_manual(values = pal_all) +
...
# calcul de sigma à partir des résidus du modèle
sigma_est <- sqrt(sum(residuals(mod1.8)^2) / nobs(mod1.8) )
