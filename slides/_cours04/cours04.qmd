...
On considère un modèle de régression linéaire gaussien avec un prédicteur continu $x$. Ce modèle contient trois paramètres à estimer : l'intercept $\alpha$, la pente $\beta$, et l'écart-type des "résidus" $\sigma$.
...
On remarque que l'effet du traitement est négligeable. Mais la présence des champignons (`fungus`) est une conséquence de l'application du `treatment`. On demande au modèle si le traitement a une influence sachant que la plante a (ou n'a pas) développé de champignons...
...
Nous avons également étendu le modèle de régression aux prédicteurs catégoriels, et introduit le concept d'interaction entre différents prédicteurs.
...
```{r ppc-mod15-3, eval = TRUE, echo = TRUE, out.width = "90%", fig.width = 14, fig.height = 7}
