...
```{r summary-mod1-hidden, eval = TRUE, echo = FALSE}
...
Ces données représentent les distributions marginales de chaque paramètre. En d'autres termes, la probabilité de $\mu$, moyennée sur toutes les valeurs possible de $\sigma$, est décrite par une distribution gaussienne avec une moyenne de `r round(post_est[1, 1], 2)` et un écart type de `r round(post_est[1, 2], 2)`. L'intervalle de crédibilité ($\neq$ intervalle de confiance) nous indique les 95% valeurs de $\mu$ ou $\sigma$ les plus probables (sachant les données et les priors).
...
```{r summary-mod4-hidden, eval = TRUE, echo = FALSE}
...
- $\alpha$ = `r round(post_est[1, 1], 2)`, 95% CrI [`r round(post_est[1, 3], 2)`, `r round(post_est[1, 4], 2)`] représente la taille moyenne quand le poids est égal à 0kg...

- $\beta$ = `r round(post_est[2, 1], 2)`, 95% CrI [`r round(post_est[2, 3], 2)`, `r round(post_est[2, 4], 2)`] nous indique qu'une augmentation de 1kg entraîne une augmentation de `r round(post_est[2, 1], 2)`cm.
...
```{r effsize-plot3, eval = TRUE, echo = TRUE, dev = "png", out.width = "75%", dpi = 200}
