...
l'après-midi, et mesure le temps de service après avoir commandé un cappuccino.
...
tous les cafés confondus) pour être servi.
...
Le fait de faire varier les intercepts par café est simplement une
...
```{r eval = TRUE, echo = TRUE}
# ensemble de packages utiles (cf. documentation: https://easystats.github.io/easystats/)
library(easystats)

# calcul de la moyenne (temps d'attente) pour matin et après-midi
estimate_means(model = mod5, by = "afternoon")
```

## Prédictions par catégorie

```{r eval = TRUE, echo = TRUE}
# représentation visuelle de l'effet de "afternoon"
...
Application des modèles généralisés additifs multi-niveaux (GAMMs) aux séries temporelles (e.g., M/EEG, iEEG, pupillometry, finger- or mouse-tracking) [@nalborczyk_precise_2025], voir le package `neurogam` (interface à `brms`): <https://lnalborczyk.github.io/neurogam/>.
...
ordinale. Ce modèle est une sorte de
