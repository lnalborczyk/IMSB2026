...
df1 %>%
  ...
  geom_text(aes(label = Loc), size = 4) +
...
data.frame(fitted(mod3) ) %>%
  ...
  # geom_smooth(
  #   data = data.frame(fitted(object = mod3) ) %>% bind_cols(data.frame(weight = d2$weight) ),
...
## ----ppc-mod15-3, eval = TRUE, echo = TRUE, out.width = "90%", fig.width = 14, fig.height = 7---------------------------
