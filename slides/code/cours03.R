...
data.frame(sample.mu, sample.sigma) %>%
    ...
    stat_density_2d(
        ...
        geom = "raster", aes(fill = after_stat(ndensity) ),
        contour = FALSE, show.legend = FALSE
        ) +
    # stat_density_2d(
    #     geom = "path",
    #     aes(group = after_stat(level) ),
    #     colour = "white",
    #     linewidth = 0.5,
    #     bins = 10
    #     ) +
    ...
    #     aes(group = after_stat(level) ),
    #     bins = 5, size = 2, padding = unit(0.05, "in"),
    #     colour = "white"
    #     ) +
    ...
    # geom_vline(xintercept = find_mode(sample.mu), lty = 2, color = "white") +
    # geom_hline(yintercept = find_mode(sample.sigma), lty = 2, color = "white") +
...
## ----summary-mod1-hidden, eval = TRUE, echo = FALSE---------------------------------------------------------------------
post_est <- posterior_summary(x = mod1, pars = c("^b_", "sigma"), probs = c(0.025, 0.975) )
...
## ----summary-mod4-hidden, eval = TRUE, echo = FALSE---------------------------------------------------------------------
post_est <- posterior_summary(x = mod4, pars = c("^b_", "sigma") )
...
## ----effsize-plot3, eval = TRUE, echo = TRUE, dev = "png", out.width = "75%", dpi = 200---------------------------------
