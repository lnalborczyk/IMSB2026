...
## Poll {background-iframe="https://embed.polleverywhere.com/multiple_choice_polls/ok3TmpBIp0fmbxGJDS7rr?controls=none&short_poll=true"}

## Slide Title {background-iframe="https://example.com"}

## {background-iframe="https://embed.polleverywhere.com/multiple_choice_polls/ok3TmpBIp0fmbxGJDS7rr?controls=none&short_poll=true"}

## {background-iframe="https://embed.polleverywhere.com/multiple_choice_polls/ok3TmpBIp0fmbxGJDS7rr?controls=none&short_poll=true" background-interactive="true"}

<iframe src="https://pollev-embeds.com/ladislasnalborczyk962" width="800px" height="600px"></iframe>

## Sondage

```{=html}
<iframe
  src="https://embed.polleverywhere.com/multiple_choice_polls/ok3TmpBIp0fmbxGJDS7rr?controls=none&short_poll=true"
  width="100%"
  height="650"
  style="border:0;"
  allow="fullscreen"
...
## {background-iframe="https://pollev-embeds.com/ladislasnalborczyk962" background-interactive="true"}

-->

## Sondage

Click here: <https://embed.polleverywhere.com/multiple_choice_polls/ok3TmpBIp0fmbxGJDS7rr?controls=none&short_poll=true>.
...
Les intervalles de confiance peuvent être interprétés comme des régions de *compatibilité* avec l'hypothèse nulle. Par conséquent, un intervalle de confiance contient la même information qu'une p-valeur et s'interprète de manière similaire.
...
Cependant, **nous ne pouvons pas conclure** qu'un intervalle de confiance en
...
il ne la contient pas [e.g., @nalborczyk_pragmatism_2019].
...
## {background-iframe="https://rpsychologist.com/d3/CI/" background-interactive="true"}
...
L'évidence en faveur d'un modèle correspond à la **vraisemblance marginale**, c'est à dire à la vraisemblance moyennée sur le prior (c'est le dénominateur du théorème de Bayes, la constante de normalisation)... Ce qui fait du facteur de Bayes un ratio de vraisemblances, pondéré par (ou moyenné sur) le prior.
...
-   À la probabilité a priori ou a posteriori d'un modèle (par
    ...
    [blogpost](https://legacy.nicebread.de/two-meanings-of-priors-1/).

-   Aux priors qu'on définit sur les paramètres d'un modèle, par exemple
    $\beta \sim \mathrm{Normal}(0, 1)$. Voir ce
    [blogpost](https://legacy.nicebread.de/two-meanings-of-priors-2/).

. . .

```{r multiple-priors, eval = TRUE, echo = FALSE, fig.align = "center", fig.width = 7.5, fig.height = 5}
...
on ne sait pas (encore) faire cela.

. . .

Par contre, on peut mesurer de combien notre incertitude est réduite en
découvrant une observation supplémentaire. Cette
...
Imaginons que nous habitions à Abu Dhabi et que la probabilité qu'il y pleuve ou qu'il y fasse beau soit de $p_{1} = 0.01$ et $p_{2} = 0.99$. L'entropie (l'incertitude) concernant la prédiction météorologique y est plus faible qu'à Grenoble.
...
## Précisions concernant la différence de divergences

Soit $p$ la "vraie distribution" et $q, r$ deux modèles candidats. La divergence de Kullback–Leibler s'écrit de la manière suivante :

$$
D_{\mathrm{KL}}(p \,\|\, q) = \sum_{i} p_{i} \log \left(\frac{p_{i}}{q_{i}}\right) = H(p, q) - H(p)
$$

avec :

- $H(p,q) = -\sum_{i} p_{i} \log (q_{i})$ : entropie croisée (i.e., à quel point le modèle $q$ est "surpris" par les données générées par $p$).
- $H(p) = -\sum_{i} p_{i} \log (p_{i})$ : entropie de la "vraie distribution".

## Différence entre deux modèles
...
Le terme $H(p)$, indépendant du modèle, **s'annule**. Comparer deux modèles via la KL **ne dépend que des termes impliquant les modèles** ($q, r$), pas de la "vraie distribution" $p$.

Mais l'entropie croisée dépend encore de $p$...

$$
H(p,q) = -\mathbb{E}_{p}[\log (q_{i})]
$$

## On remplace $p$ par les données

On observe un ensemble d'observations indépendantes et identiquement distribuées :

$$
x_{1}, \dots, x_{n} \sim p
$$

Par la loi des grands nombres (i.e., la fréquence empirique approxime l'espérance) :

$$
\mathbb{E}_{p}[\log (q(X))]
\;\approx\;
\frac{1}{n}\sum_{i=1}^n \log q(x_{i})
$$

Autrement dit, l'échantillon joue le rôle de $p$ !

## Estimation empirique de l'entropie croisée
...
Ce terme est entièrement calculable, ne dépend uniquement du modèle $q$, et est évalué sur les données observées. Lorsqu'on compare deux modèles, on peut donc faire une "simple" différence de **log-vraisemblances moyennes**.
...
&\approx
-\frac{1}{n}\sum_{i=1}^n \log q(x_{i})
+ \frac{1}{n}\sum_{i=1}^n \log r(x_{i})
\end{aligned}
$$

## Vers la déviance (exemple d'application #1)

Exemple d'une distribution cible uniforme $p$, et 4 modèles $q1, q2, q3, q4$ (exemple tiré de [Wikipedia](https://en.wikipedia.org/wiki/Kullback–Leibler_divergence#Basic_example)).
...
## Vers la déviance (exemple d'application #2)

Deuxième exemple, distribution cible et modèles Gaussiens, on utilise les données comme approximation de $p$.
...
La déviance a le même problème que le $R^{2}$, lorsqu'elle est calculée sur l'échantillon observé (i.e., l'échantillon d'entraînement). Dans ce cas, on l'appelle déviance **in-sample**.

. . .

Si on est intéressé par les capacités de prédiction de notre modèle, nous pouvons calculer la déviance du modèle sur de nouvelles données (données de test)... qu'on appellera dans ce cas déviance **out-of-sample**. Cela revient à se demander si notre modèle est performant pour prédire de nouvelles données (non observées).
...
Une condition d'application de l'AIC et du DIC est que la distribution a posteriori soit une distribution gaussienne multivariée. Le WAIC relâche cette condition et donne généralement des estimations plus précises de la déviance *ouf-of-sample*.

. . .

Un aspect important du WAIC est qu'il est dit "pointwise", c'est à dire qu'il considère l'imprécision de prédiction point par point (donnée par donnée), pour chaque observation.
...
critères d'information mais aussi les outils de **posterior predictive
...
outils comme la validation croisée ou les critères d'information
