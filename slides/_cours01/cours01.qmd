...
**Règle de la somme** (pour deux événements mutuellement exclusifs) : $\Pr(A_{1} \cup A_{2}) = \Pr(A_{1}) + \Pr(A_{2}) - \Pr(A_{1} \cap A_{2})$.
...
**Règle du produit** (pour deux événements indépendants) : $\Pr(A_{1} \cap A_{2}) = \Pr(A_{1}) \times \Pr(A_{2})$.
...
Lords du lancer de 2 dés, la probabilité que le dé $x$ soit égal à 2 et que le dé $y$ soit égal à 3 est : $\Pr(x = 2, y = 3) = \Pr(y = 3, x = 2) = \frac{1}{36}$.
...
Quelle est la probabilité que le dé $x$ ait une certaine valeur **sachant que** le total est égal à 4 ? Par exemple, la probabilité que le dé $x$ soit égal à 2 (sachant que le total est 4) : $\Pr(x = 2 \given x + y = 4) = \frac{1}{3}$.
...
Cette probabilité conditionnelle peut être réécrite : $\Pr(x = 2 \given x + y = 4) = \frac{\Pr(x = 2, x + y = 4) }{\Pr(x + y = 4) } = \frac{1/36}{3/36} = \frac{1}{3}$. Notez que $\Pr(x \given y)$ **n'est pas nécessairement égal** (et n'est généralement pas égal) à $\Pr(y \given x)$.

. . .

Par exemple : la probabilité de mourir en sachant que vous avez été attaqué par un requin n'est pas la même que la probabilité d'avoir été attaqué par un requin en sachant que vous êtes mort ([confusion de l'inverse](https://en.wikipedia.org/wiki/Confusion_of_the_inverse)). De la même manière, $p(\text{données} \given \mathcal{H}_{0}) \neq p(\mathcal{H}_{0} \given \text{données})$.
...
Par exemple, $\Pr(x = 2, y = \text{pair}) = \dfrac{\frac{3}{36}}{\frac{18}{36}} \times \frac{18}{36} = \dfrac{\frac{3}{36}}{\frac{3}{36}} \times \frac{3}{36} = \frac{3}{36} \approx  0.0833$.

```{r marginalisation-plot4, out.width = "35%"}
#| echo: FALSE
#| fig-width: 6
#| fig-height: 6
crossing(X = 1:6, Y = 1:6) %>%
...
![](figures/wrong.gif){fig-align="center" width="50%"}
...
On remarque à nouveau que $\Pr(\text{cheveux = blonds} \given \text{yeux = bleus})$ **n'est pas nécessairement égal** (et n'est généralement pas égal) à $\Pr(\text{yeux = bleus} \given \text{cheveux = blonds})$.
...
{{< video figures/montyhall.mp4 width="1200" height="600" >}}
